...
The potential energy surface for a diatomic molecule \ce{A-B} is easy to visualize, because the energy depends only on the bond distance $r$, that is, $E = E(r)$.
...
In the first case (\textbf{A}), the reaction \ce{A + B} $\rightarrow$ {AB} is barrierless and happens spontaneously.
...
In the second case  (\textbf{B}),  the reaction \ce{A + B} $\rightarrow$ {AB} proceeds via a transition state \ce{AB}* that has an energy higher than \ce{A + B}.
